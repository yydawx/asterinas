...
impl BindPortConfig {
    ...
    pub(super) fn can_reuse(&self) -> bool {
        matches!(self, Self::CanReuse(_, _)) || matches!(self, Self::Ephemeral(_, true))
    }

    pub(super) fn port(&self) -> Option<u16> {
        match self {
            Self::CanReuse(_, port) | Self::Specified(_, port) | Self::Backlog(_, port) => {
                Some(*port)
            }
    ...
    pub(super) fn addr(&self) -> IpAddress {
        match self {
            Self::CanReuse(addr, _)
            | Self::Specified(addr, _)
            | Self::Ephemeral(addr, _)
            | Self::Backlog(addr, _) => *addr,
        }
    }
}
