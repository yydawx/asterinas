...
use super::{
    addr::{IpAddressFamily, SocketFamily, resolve_remote_endpoint},
...
impl Socket for StreamSocket {
    ...
    fn connect(&self, socket_addr: SocketAddr) -> Result<()> {
        ...
        debug!("connect: after try_into = {:?}", remote_endpoint);
        let remote_endpoint = self.prepare_endpoint(remote_endpoint)?;
        debug!("connect: after prepare = {:?}", remote_endpoint);
        let remote_endpoint = resolve_remote_endpoint(self.family, remote_endpoint);
        debug!("connect: after resolve = {:?}", remote_endpoint);
        if let Some(result) = self.start_connect(&remote_endpoint) {
            debug!("connect: start_connect returned Err({:?})", result);
            return result;
        }
        debug!("connect: blocking wait");
